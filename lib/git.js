import { execSync, execFileSync, exec as execCb } from 'child_process';
      ...
      try {
        ...
        for (const stepSessionId of getStepSessionIdCandidates(c.sessionId, session)) {
          if (canEnrich && !enrichedFiles) {
            enrichedFiles = (c.files || []).map(f => {
              if (f.binary) return { ...f, commitContent: null, addedLines: null };
              const inputs = getCommitFileBlameInputs(c.repo, c.hash, f.path);
              return { ...f, ...inputs };
            });
          }
