...
export function detectAICommit(subject = '', author = '', body = '') {
  ...
  if (signals.length > 0) {
    // Derive detectedTool: prefer non-generic-ai signals first
    let detectedTool = null;
    for (const sig of signals) {
      const tool = SIGNAL_TO_TOOL[sig];
      if (tool && tool !== 'generic-ai') {
        detectedTool = tool;
        break;
      }
    }
...
export function computeAIContribution(commits, toolFilter = null) {
  ...
  const filteredCommits = toolFilter
    ? (commits || []).filter(c => c.attributedTool === toolFilter)
    : (commits || []);
...
export function finalizeGitStats(merged, sessions = []) {
  ...
  merged.aiContributionByTool = {
    claude: computeAIContribution(merged.commitList, 'claude'),
    codex: computeAIContribution(merged.commitList, 'codex'),
    opencode: computeAIContribution(merged.commitList, 'opencode'),
    'generic-ai': computeAIContribution(merged.commitList, 'generic-ai'),
  };
