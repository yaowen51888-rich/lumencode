...
export class StepTracker {
  ...
  getLineAttributionForCommit(commit) {
    ...
    for (const file of commit.files || []) {
      ...
      const stepContent = this.db.getFileBlob(latestStep.step_id, filePath);
      const aligned = typeof file.commitContent === 'string'
        && stepContent !== null
        && stepContent === file.commitContent
        && Array.isArray(file.addedLines)
      ...
      if (aligned) {
        ...
        for (const n of file.addedLines) {
          const idx = n - 1;
          if (idx >= 0 && idx < blameMap.lines.length && stepIds.has(blameMap.lines[idx])) {
            ai++;
          } else {
            human++;
          }
