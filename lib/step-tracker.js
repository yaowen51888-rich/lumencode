...
import { computeBlame, buildInitialBlameMap, projectStepBlameToCommit, projectStepBlameToCommitPerLine, UNKNOWN_BLAME } from './line-blame.js';
import {
...
export function extractTargetFiles(toolName, toolInput, projectRoot) {
  const files = [];
  ...
  if (['write', 'edit', 'multiedit', 'notebookedit', 'file_write', 'file_edit', 'write_file', 'edit_file', 'replace'].includes(normalizedTool)) {
    ...
    // codex 等工具经 shell 改文件：提取重定向目标 + 常见改文件命令的目标参数。
    // ponytail: shell 解析本质不可靠，只覆盖主流模式；下游 normalizePath + recordStep 的 existsSync 兜底过滤假阳性。
    const rawCmd = input.command || input.cmd || input.run || (typeof input === 'string' ? input : '');
    const joined = Array.isArray(rawCmd) ? rawCmd.join(' ') : (rawCmd || '');
    // 解包 sh -c "..." / sh -c '...' / sh -c <cmd>（codex shell 命令常以此形式传入）
    const qsh = joined.match(/\bsh\s+-c\s+(['"])([\s\S]*?)\1/);
    const psh = !qsh && joined.match(/\bsh\s+-c\s+(.+)$/);
    const cmd = qsh ? qsh[2] : (psh ? psh[1] : joined);
    if (cmd) {
      // 重定向 > / >> 目标
      ...
      // sed -i / cp / mv / tee / install / rsync / truncate / dd：目标通常为末尾非选项参数
      const segs = cmd.split(/[;&|]+/).map(s => s.trim()).filter(Boolean);
      const cmdHeadRe = /^(?:sudo\s+)?(?:\S+\/)?(sed|cp|mv|tee|install|rsync|truncate|dd)\b\s*(.*)$/i;
      for (const seg of segs) {
        const hm = seg.match(cmdHeadRe);
        if (!hm) continue;
        const rest = hm[2];
        const ofm = rest.match(/\bof=('|"|)([^'"\s]+)\1/);
        if (ofm) { const p = normalizePath(ofm[2], projectRoot); if (p) files.push(p); }
        const toks = rest.match(/['"]?([^'"\s=]+)['"]?/g) || [];
        const paths = toks.map(t => t.replace(/^['"]|['"]$/g, '')).filter(t => t && !t.startsWith('-') && t !== '/');
        if (paths.length) {
          const p = normalizePath(paths[paths.length - 1], projectRoot);
          if (p) files.push(p);
        }
      }
    }
  ...
  getLineAttributionForCommit(commit) {
    ...
    for (const file of commit.files || []) {
      ...
      if (aligned) {
        ...
        for (const n of file.addedLines) {
          ...
          if (idx >= 0 && idx < blameMap.lines.length) {
            ...
            if (!lineBlame || lineBlame === UNKNOWN_BLAME) { unknown++; lineClassifications.push('unknown'); }
            else if (stepIds.has(lineBlame)) { ai++; lineClassifications.push('ai'); }
            else { human++; lineClassifications.push('human'); }
          } else {
            unknown++;
            lineClassifications.push('unknown');
          }
        }
        fileAIAdded = ai;
        ...
        if (fuzzy.coverage >= this.fuzzyCoverageThreshold) {
          ...
          lineClassifications = projectStepBlameToCommitPerLine(stepContent, commitContentN, blameMap, file.addedLines || [], stepIds);
        } else {
          result.degradedFiles++;
      ...
      result.fileBreakdown[filePath] = {
        ...
        method,
        lines: (file.addedLines || []).map((newLine, index) => {
          const stepId = method === 'aligned' ? blameMap.lines[newLine - 1] : null;
          const step = stepId && stepId !== UNKNOWN_BLAME ? this.db.getStepById(stepId) : null;
          return {
            newLine, classification: lineClassifications[index] || 'unknown',
            stepId: step?.id || null, sessionId: step?.session_id || latestStep.session_id || null,
            tool: step?.tool_name || latestStep.tool_name || null,
            confidence: method === 'aligned' ? 'high' : method === 'fuzzy' ? 'medium' : 'low',
            reason: method === 'fallback' ? 'aggregate_fallback' : 'step_blame',
          };
        }),
      };
    }
