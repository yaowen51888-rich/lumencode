...
const MODEL_PRICING = {
  ...
  // OpenAI 模型 (Codex / OpenCode)
  'gpt-4.1':         { input: 2,    output: 8,    cacheRead: 0.50, cacheCreate: 0 },
  'gpt-4.1-mini':    { input: 0.40, output: 1.60, cacheRead: 0.10, cacheCreate: 0 },
  'gpt-4.1-nano':    { input: 0.10, output: 0.40, cacheRead: 0.025, cacheCreate: 0 },
  'gpt-5':           { input: 5,    output: 20,   cacheRead: 1.25, cacheCreate: 0 },
  'o3':              { input: 2,    output: 8,    cacheRead: 0.50, cacheCreate: 0 },
  'o3-pro':          { input: 20,   output: 80,   cacheRead: 5,    cacheCreate: 0 },
  'o4-mini':         { input: 1.10, output: 4.40, cacheRead: 0.275, cacheCreate: 0 },
};
...
export function resolveModelPricing(model) {
  ...
  for (const [key, pricing] of Object.entries(MODEL_PRICING)) {
    ...
    if (lower.includes('gpt-4.1-mini') && key.includes('gpt-4.1-mini')) return pricing;
    if (lower.includes('gpt-4.1-nano') && key.includes('gpt-4.1-nano')) return pricing;
    if (lower.includes('gpt-4.1') && key.includes('gpt-4.1')) return pricing;
    if (lower.includes('gpt-5') && key.includes('gpt-5')) return pricing;
    if (lower.includes('o3-pro') && key.includes('o3-pro')) return pricing;
    if (lower.includes('o3') && key.includes('o3') && !key.includes('pro')) return pricing;
    if (lower.includes('o4-mini') && key.includes('o4-mini')) return pricing;
  }
