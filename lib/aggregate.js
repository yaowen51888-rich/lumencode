...
export function computeUsageStats(records, scenarioKeywords, costMode = 'auto') {
  const stats = {
    sessionCount: new Set(records.filter(r => !r.isSubagent).map(r => r.sessionId).filter(Boolean)).size,
  ...
  for (const r of records) {
    ...
    if (r.type === 'assistant') {
      ...
      const tokenTotal = r.tokens.input + r.tokens.output + r.tokens.cacheRead + r.tokens.cacheCreate;
      stats.totalTokens += tokenTotal;
      if (r.isSubagent) stats.subagentTokens += tokenTotal;
