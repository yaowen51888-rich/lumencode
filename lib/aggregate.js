...
// ponytail: 单会话 token 爆表阈值（硬编码）；验证有用后再做 config 项
const HEAVY_SESSION_THRESHOLD = 1_000_000;
...
export function deduplicateRecords(records) {
  ...
  // 多级稳定排序：timestamp 相同时按 tool/sessionId/model/token 逐级 tie-break，
  // 消除 readdir 顺序漂移导致的 cost 浮点累加非确定性（殃及智能报告 sourceHash 稳定性）
  deduped.sort((a, b) => {
    const ka = [a.timestamp || '', a.tool || '', a.sessionId || '', a.model || '', a.inputTokens || 0, a.outputTokens || 0];
    const kb = [b.timestamp || '', b.tool || '', b.sessionId || '', b.model || '', b.inputTokens || 0, b.outputTokens || 0];
...
export function computeUsageStats(records, scenarioKeywords, costMode = 'auto') {
  // ponytail: 单遍历 — session/activeDays/request/userMsg 计数全部折进下方主循环，
  // 避免对 records 多次 .filter/.map 全量扫描（日志量万级，省 O(4n)）
  const sessTokenAccum = {};
  ...
  for (const r of records) {
    ...
    const isSubagent = r.metadata?.isSubagent || r.isSubagent;

    // 折入主循环的 4 项计数（原先各自一次独立 filter 扫描）
    if (isAssistant) stats.requestCount++;
    if (!isSubagent && r.sessionId) sessionSet.add(r.sessionId);
    if (r.timestamp) activeDaysSet.add(r.timestamp.slice(0, 10));
    const userType = r.metadata?.type || r.type;
    const userText = r.metadata?.text || r.text || '';
    if (userType === 'user' && userText && !userText.startsWith('<system-reminder')) stats.userMessageCount++;
    ...
    if (isAssistant) {
      ...
      // 子代理 token 累到父 sessionId（树总），与 groupBySessions 父含子孙口径一致
      const targetSessId = r.parentSessionId || r.sessionId;
      if (targetSessId) sessTokenAccum[targetSessId] = (sessTokenAccum[targetSessId] || 0) + tokenTotal;
      if (isSubagent) stats.subagentTokens += tokenTotal;
    ...
    if (!stats.toolBreakdown[toolName]) {
      stats.toolBreakdown[toolName] = { inputTokens: 0, outputTokens: 0, cacheRead: 0, cacheCreate: 0, count: 0, sessionIds: new Set() };
    }
    if (isAssistant) {
    ...
    // 每工具 session 计数：与全局 sessionSet 同口径（排除 subagent），
    // 供 coverage 卡按"本周期"判断 active，而非全量历史
    if (!isSubagent && r.sessionId) stats.toolBreakdown[toolName].sessionIds.add(r.sessionId);
    ...
    if (projectName) {
      ...
      if (isAssistant) {
        ...
        const recCost = (r.costUSD != null && r.costUSD > 0) ? r.costUSD : calculateRecordCost(r, pricing).cost;
    ...
    if (r.timestamp) {
      ...
      // 注意：此处用 meta==='user' || type==='user' 双判，与上方 userMessageCount 用的
      // coalescedType === 'user'（metadata?.type || type）语义不同——后者当 metadata.type
      // 非 'user' 但 r.type==='user' 时会漏计。保持各自原语义，勿 DRY 合并。
  ...
  // toolBreakdown 的 sessionIds 收口为 sessionCount（Set 不便序列化，且消费者只读 count）
  for (const tb of Object.values(stats.toolBreakdown)) {
    tb.sessionCount = tb.sessionIds.size;
    delete tb.sessionIds;
  }

  // Scenarios
  stats.scenarios = aggregateScenarios(records, scenarioKeywords);

  // Cost estimation（顺带产出 cacheSaving，避免再遍历一次）
  const { cost: estimatedCost, cacheSaving } = computeCostBreakdownFromRecords(records, costMode);
  stats.estimatedCost = Math.round(estimatedCost * 100) / 100;
  stats.cacheSaving = Math.round(cacheSaving * 100) / 100;
  ...
  const unpricedTokens = unknownModels.reduce((sum, model) => {
    const data = stats.models[model];
    return sum + data.inputTokens + data.outputTokens + data.cacheRead + data.cacheCreate;
  }, 0);
  stats.costMeta = {
    ...
    unknownModels,
    unpricedTokens,
    unpricedRatio: stats.totalTokens > 0 ? unpricedTokens / stats.totalTokens : 0,
    hasActualCost,
  };
  ...
  for (const r of records) {
    ...
    if (costMode === 'display') {
      ...
      stats.models[model].cost += calculateRecordCost(r, pricing).cost;
    } else {
  ...
  stats.heavySessionCount = Object.values(sessTokenAccum).filter(t => t > HEAVY_SESSION_THRESHOLD).length;
  stats.warnSessionCount = Object.values(sessTokenAccum).filter(t => t > HEAVY_SESSION_THRESHOLD / 2 && t <= HEAVY_SESSION_THRESHOLD).length;
  return stats;
}

// 日期分桶索引：单遍历建 Map<date, records[]> + 升序日期数组。
// WeakMap 按记录数组实例缓存——同 allRecords 跨请求复用，数组 GC 即回收，零失效逻辑。
// 用途：filterRecordsByPeriod / computeTrendData 按日期键查询，免去每次请求全量扫描。
const _dateIndexCache = new WeakMap();
function buildDateIndex(records) {
  ...
  const sorted = [...byDate.keys()].sort(); // YYYY-MM-DD 字典序 = 日期序
  return { byDate, sorted };
}
function getDateIndex(records) {
  let idx = _dateIndexCache.get(records);
  if (!idx) { idx = buildDateIndex(records); _dateIndexCache.set(records, idx); }
  return idx;
}

/**
 * 按周期过滤记录，返回 { filtered, start, end }。
 * filtered 按【日期升序】排列（日期索引桶拼接），非原始文件读取序——下游 computeUsageStats
 * 纯累加，无序敏感。若新增依赖原始顺序的消费者，需在此重排或回退为稳定序扫描。
 */
export function filterRecordsByPeriod(records, period, refDate, options = {}) {
  ...
  // 日期索引查询替代全量扫描：按日期桶收集 [start,end] 内记录。
  // O(活跃天数 + 匹配记录) 而非 O(总记录数)；输出按日期升序（computeUsageStats 无序敏感，安全）。
  const { byDate, sorted } = getDateIndex(records);
  const filtered = [];
  for (const d of sorted) {
    if (d < start) continue;
    if (d > end) break; // 升序，越界即止
    const arr = byDate.get(d);
    for (let i = 0; i < arr.length; i++) filtered.push(arr[i]);
  }

  return { filtered, start, end };
}

// 跨天归因回溯：构建"报告期开始前 N 天"的上下文 session，仅作 commit→session 匹配候选。
// 场景：Day N 撰写、Day N+1 提交——日报只载本日 session 会漏掉撰写会话，
// 提交被判 NONE 稀释 AI%。时间窗 [start - backDays, start)，与主筛选同 tool/project 范围。
// ponytail: 返回 session 仅供 matcher 候选，不参与展示/聚合；调用方按 id 去重合并。
export function buildAttributionContextSessions(records, { start, backDays = 3, tool = null, projectBases = null } = {}) {
  const startMs = Date.parse(`${start}T00:00:00`);
  if (!Number.isFinite(startMs)) return [];
  const days = Number(backDays);
  const back = Number.isFinite(days) && days > 0 ? days : 3;
  const ctxStart = new Date(startMs);
  ctxStart.setDate(ctxStart.getDate() - back);
  const ctxStartStr = ctxStart.toISOString().slice(0, 10);
  const baseSet = projectBases && projectBases.size ? projectBases : null;
  const ctx = [];
  for (const r of records || []) {
    const d = r.timestamp?.slice(0, 10);
    if (!d || d >= start || d < ctxStartStr) continue; // 严格早于报告期 start
    if (tool && r.tool !== tool) continue;
    if (baseSet && !baseSet.has(getProjectBaseName(r.project))) continue;
    ctx.push(r);
  }
  return groupBySessions(ctx);
}
...
export function computeTrendData(allRecords, period, refDate) {
  ...
  // 日期索引查询：仅遍历 [trendStart,trendEnd] 内的日期桶，非全量 allRecords。
  const { byDate, sorted } = getDateIndex(allRecords);
  const dailyStats = {};
  for (const d of sorted) {
    if (d < trendStart || d > trendEnd) continue;
    const ds = { requests: 0, inputTokens: 0, outputTokens: 0, tools: {} };
    for (const r of byDate.get(d)) {
      if (isAssistantRecord(r)) {
        ds.requests++;
        ds.inputTokens += getInputTokens(r);
        ds.outputTokens += getOutputTokens(r);
      }
      const toolCalls = r.metadata?.toolCalls || r.toolCalls || [];
      for (const tc of toolCalls) {
        if (tc.name === 'Skill' || tc.name?.startsWith('mcp__')) continue;
        if (!ds.tools[tc.name]) ds.tools[tc.name] = { calls: 0, uses: 0 };
        ds.tools[tc.name].calls++;
      }
      const uniqueNames = new Set(toolCalls.map(tc => tc.name).filter(n => n !== 'Skill' && !n?.startsWith('mcp__')));
      for (const name of uniqueNames) {
        if (!ds.tools[name]) ds.tools[name] = { calls: 0, uses: 0 };
        ds.tools[name].uses++;
      }
    }
    dailyStats[d] = ds;
...
export function groupBySessions(records) {
  ...
  for (const r of records) {
    ...
    if (isAssistant) {
      ...
      s.cacheRead += getCacheRead(r);
      s.cacheCreate += getCacheCreate(r);
  ...
  const arr = Object.values(sessions).map(s => {
    ...
    const totalTokens = s.inputTokens + s.outputTokens + s.cacheRead + s.cacheCreate;
    const isHeavy = totalTokens > HEAVY_SESSION_THRESHOLD;
    return {
      ...rest,
      models: [...s.models],
      primaryTool,
      totalTokens,
      isHeavy,
      isWarn: !isHeavy && totalTokens > HEAVY_SESSION_THRESHOLD / 2,
    };
  });

  // 建会话树：子 session（有 parentSessionId）挂到父摘要；父 totalTokens 累成树总，
  // 兼容 P0-B 爆表基数（父含子孙）。孤儿子（父不在集）保留顶层当根处理。
  const byId = new Map(arr.map(s => [s.id, s]));
  const attached = new Set();
  for (const s of arr) {
    if (!s.parentSessionId || s.id === s.parentSessionId) continue;
    const parent = byId.get(s.parentSessionId);
    // 仅挂到根会话（父本身无 parentSessionId），杜绝 A→B→A 循环引用；孤儿子（父缺失/父非根）保留顶层当根
    if (!parent || parent.parentSessionId) continue;
    parent.children.push({
      id: s.id, totalTokens: s.totalTokens, isHeavy: s.isHeavy, isWarn: s.isWarn,
      primaryTool: s.primaryTool, requests: s.requests,
    });
    parent.totalTokens += s.totalTokens;
    parent.isHeavy = parent.totalTokens > HEAVY_SESSION_THRESHOLD;
    parent.isWarn = !parent.isHeavy && parent.totalTokens > HEAVY_SESSION_THRESHOLD / 2;
    attached.add(s.id);
  }
  return arr.filter(s => !attached.has(s.id)).sort((a, b) => b.endTime.localeCompare(a.endTime));
}
...
function calculateRecordCost(record, pricing) {
  if (pricing.unknown) return { cost: 0, saving: 0 };
  ...
  // 缓存命中节省：cacheRead 本该按 input 价付，实际按 cacheRead 价付，差额即省
  const saving = (cacheRead / 1_000_000) * (inputRate - cacheReadRate);

  return { cost, saving };
}

export function computeCostBreakdownFromRecords(records, costMode = 'auto') {
  let cost = 0;
  let cacheSaving = 0;
  for (const r of records) {
    if (!isAssistantRecord(r)) continue;
    ...
    if (costMode === 'display') {
      ...
      const rec = calculateRecordCost(r, pricing);
      cost += rec.cost;
      cacheSaving += rec.saving;
    } else {
      cost += costUSD;
    }
  }
  return { cost, cacheSaving };
}

export function computeCostFromRecords(records, costMode = 'auto') {
  return computeCostBreakdownFromRecords(records, costMode).cost;
}
