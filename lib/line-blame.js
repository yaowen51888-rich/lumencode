...
export function projectStepBlameToCommit(stepContent, commitContent, stepBlameMap, addedLines, stepIds) {
  ...
  if (added.length === 0) return { aiAdded: 0, humanAdded: 0, coverage: 0 };
  if (!stepContent || !commitContent || !stepBlameMap || !stepBlameMap.lines) {
    return { aiAdded: 0, humanAdded: added.length, coverage: 0 };
  }
  ...
  // commit 1-based 行号 → step 0-based 行索引（仅 equal 区间可映射）
  const commitToStep = new Map();
  for (const op of opcodes) {
    if (op.tag !== 'equal') continue;
    for (let j = op.newStart; j < op.newEnd; j++) {
      commitToStep.set(j + 1, op.oldStart + (j - op.newStart));
    }
  }
  ...
  for (const lineNo of added) {
    ...
    if (stepIdx !== undefined && stepIdx >= 0 && stepIdx < stepBlameMap.lines.length) {
      mapped++;
      if (stepIds.has(stepBlameMap.lines[stepIdx])) aiAdded++;
      else humanAdded++;
    } else {
      // 未映射（insert/replace 区间或越界）：保守计 human，保 ai+human=added 守恒
      humanAdded++;
    }
  }

  return { aiAdded, humanAdded, coverage: mapped / added.length };
}
