...
export class CopilotParser extends BaseParser {
  ...
  // OTel 多源 timestamp：[sec,nanos] 数组 | 标量（自适应秒/毫秒/微秒/纳秒）| timeUnixNano
  _timestampFrom(obj) {
    ...
    for (const key of ['timestamp', 'observedTimestamp']) {
      ...
      if (typeof v === 'number' && v > 0) {
        let ms;
        if (v >= 1e17) ms = v / 1e6;       // ns
        else if (v >= 1e14) ms = v / 1e3;  // μs
        else if (v >= 1e11) ms = v;        // ms
        else ms = v * 1e3;                 // s
        return new Date(ms).toISOString();
      }
    }
    const nano = num(obj.timeUnixNano);
    if (nano > 0) return new Date(nano / 1e6).toISOString();
...
function splitPaths(raw) { return String(raw).split(',').map(s => s.trim()).filter(s => s !== ''); }
