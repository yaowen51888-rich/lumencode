...
export class ClaudeParser extends BaseParser {
  ...
  _convertToUsageRecord(raw, projectDir, fallbackSessionId = '') {
    ...
    if (raw.cwd) {
      const gitRoot = resolveGitRoot(raw.cwd);
      // 仅当 cwd 推导的 git 根仍真实存在时采用；否则项目已 rename/move（旧 cwd 失效），
      // 回退 Claude 项目目录名，避免旧 session 因旧路径被项目过滤排除 + matcher 失配而断链。
      if (gitRoot && existsSync(gitRoot)) projectName = gitRoot;
    }
