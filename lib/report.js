...
function buildGitNarrative(git, periodName) {
  ...
  if (git.aiContribution && git.commits > 0) {
    ...
    line += `AI 代码变更占比 ${aiLinePct}%（新增 +${fmtN(ai.aiLinesAdded || 0)} / 删除 -${fmtN(ai.aiLinesDeleted || 0)}）`;
...
export function generateFeishuCard(usageData, gitData, period, startDate, endDate, tool) {
  ...
  if (usageData.estimatedCost || usageData.costMeta?.unknownModels?.length) {
  ...
  if (gitData && gitData.commits > 0) {
    ...
    if (gitData.aiContribution && gitData.commits >= 3) {
      ...
      if ((ai.aiLineRatio ?? ai.aiRatio ?? 0) > 0) {
        const aiAdded = ai.aiLinesAdded || 0;
        const aiDeleted = ai.aiLinesDeleted || 0;
        lines.push(bullet(`AI 代码变更占比 ${linePct}%，新增 +${formatInt(aiAdded)} / 删除 -${formatInt(aiDeleted)} 行`));
      }
    }
    lines.push('');
  }
  ...
  if (usageData.estimatedCost || usageData.costMeta?.unknownModels?.length) {
    ...
    if (usageData.costMeta) {
      ...
      if (meta.unknownModels?.length > 0) {
        const ratio = ((meta.unpricedRatio || 0) * 100).toFixed(1);
        notes.push(`${formatInt(meta.unpricedTokens || 0)} token（${ratio}%，${meta.unknownModels.length} 个模型）未计价：${meta.unknownModels.join('、')}；可在 data/pricing.json 的 overrides 中配置 aliasOf`);
      }
      if (notes.length > 0) {
