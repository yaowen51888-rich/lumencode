...
export function generateFeishuCard(usageData, gitData, period, startDate, endDate) {
  ...
  if (gitData?.commits > 0) {
    ...
    if (gitData.aiContribution) {
      const ai = gitData.aiContribution;
      const pct = Math.round((ai.aiCommits / gitData.commits) * 100);
      fields.push({ is_short: true, text: { tag: 'lark_md', content: `**AI 占比**\n${pct}%` } });
    }
  }
  ...
  if (gitData && gitData.commits > 0) {
    ...
    if (gitData.aiContribution && gitData.commits > 0) {
      ...
      if (pct > 0) {
        ...
        lines.push(bullet(`AI 协作占比 ${pct}%，涉及 +${formatInt(aiAdded)}/-${formatInt(aiDeleted)} 行`));
      }
    }
    lines.push('');
  }
  ...
  if (gitData && (gitData.commits > 0 || gitData.filesChanged > 0)) {
    ...
    if (gitData.commitList?.length) {
      ...
      if (aiDetail) {
        ...
        lines.push(`- 高/中置信 AI 提交 **${totalAI}/${totalCommits}**（${Math.round(totalAI / totalCommits * 100)}%），涉及 +${formatInt(aiDetail.totalAIFileAdded)}/-${formatInt(aiDetail.totalAIFileDeleted)} 行`);
