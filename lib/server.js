...
export function startServer(config, effectiveIncludeProjects, buildReportData, configPath) {
  ...
  function parseHookTools(value) {
    if (!value) return [HOOK_TOOLS.CLAUDE, HOOK_TOOLS.CODEX];
    const tools = [];
  ...
  const server = createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/hooks') {
      try {
        if (req.method === 'GET') {
          writeJson(res, 200, getHooksStatus(process.cwd()));
          return;
        }
        ...
        if (req.method === 'POST') {
          readJsonBody(req, res, async (body) => {
            ...
            if (action !== 'disable') {
              stepTracking = await initStepTracking(process.cwd());
            }
            const results = action === 'disable'
              ? disableHooks(process.cwd(), tools, { backup: true })
              : enableHooks(process.cwd(), tools, { backup: true });
            writeJson(res, 200, {
              success: true,
              action,
              stepTracking,
              results,
              status: getHooksStatus(process.cwd()),
            });
          });
          return;
        }
  ...
  server.listen(PORT, '127.0.0.1', () => {
    ...
    const hookStatus = getHooksStatus(process.cwd());
    const hookParts = [
      `Claude ${hookStatus.claude.enabled ? 'on' : 'off'}`,
      `Codex ${hookStatus.codex.enabled ? 'on' : 'off'}`,
      `steps ${hookStatus.stepsInitialized ? 'ready' : 'not initialized'}`,
    ];
    process.stdout.write(`  ${dim}●${R}  ${B}Hooks${R}       ${hookParts.join(' / ')}\n`);
    if (!hookStatus.claude.enabled || !hookStatus.codex.enabled || !hookStatus.stepsInitialized) {
      process.stdout.write(`  ${yellow}${B}!${R}  行级归因未完整开启：运行 ${B}node index.js hooks enable${R}，或在页面中开启。\n`);
    }
    process.stdout.write('\n');
