            ...
            if (config.repos?.length > 0) {
              ...
              if (matchedRepo) {
                try {
                  ...
                  repoGit = await finalizeGitStats(repoGit, sessions, { attribution: config.aiAttribution });
        ...
        if (config.repos?.length) {
          try {
            ...
            if (sessionRepos.length > 0) {
              ...
              await finalizeGitStats(gitStats, sessions, { attribution: config.aiAttribution });
            }
          } catch (e) { console.warn("[server] error", e.message); }
        }
