...
import { createSmartReport, detectSmartReportAgents } from './smart-report.js';
  ...
  const server = createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/smart-report') {
      if (req.method === 'GET') {
        ...
        const record = readSmartReportRecord(storeDir, buildSmartReportKey(query));
      ...
      readJsonBody(req, res, async (body) => {
        ...
        if (!VALID_PERIODS.includes(period)) {
          writeJson(res, 400, { error: `无效的 period 参数，可选值：${VALID_PERIODS.join('/')}` });
          return;
        }

        const date = body.date || new Date().toISOString().slice(0, 10);
        const tool = body.tool || 'all';
        const agent = body.agent || '';
        const customStart = body.start || '';
        const customEnd = body.end || '';
        const platform = body.platform || 'default';
        const level = body.level || 'detailed';
        ...
        if (period === 'custom') {
          if (!customStart || !customEnd || !/^\d{4}-\d{2}-\d{2}$/.test(customStart) || !/^\d{4}-\d{2}-\d{2}$/.test(customEnd)) {
            writeJson(res, 400, { error: '自定义周期需要 start 和 end 参数 (YYYY-MM-DD)' });
            return;
          }
          if (customStart > customEnd) {
            writeJson(res, 400, { error: '起始日期不能晚于结束日期' });
            return;
          }
        }

        let parsed = await getOrParse(config, includeProjects);
        const data = await buildReportData(period, date, config, includeProjects, tool, parsed, { customStart, customEnd });
        if (!data) {
        ...
        if (project) {
          ...
          const projRecords = toolRecords.filter(r => getProjectBaseName(r.project) === project);
          ...
          workPrevStats = null;
          workGitStats = deriveProjectGitStats(data.gitStats, project, pStart, pEnd, config.aiAttribution);
          projectName = project;
          reportData = {
            ...data,
        ...
        const detailedMarkdown = generateWorkReport(workUsageStats, workGitStats, period, data.start, data.end, workPrevStats, { level: 'detailed', platform, tool, projectName });
        const briefMarkdown = generateWorkReport(workUsageStats, workGitStats, period, data.start, data.end, workPrevStats, { level: 'brief', platform, tool, projectName });
        const bossMarkdown = generateBossReport(workUsageStats, workGitStats, period, data.start, data.end, workPrevStats, platform);
        const workMarkdown = level === 'boss' ? bossMarkdown : level === 'brief' ? briefMarkdown : detailedMarkdown;
        const sourceReports = {
          detailedMarkdown,
          briefMarkdown,
          bossMarkdown: level === 'boss' ? bossMarkdown : '',
        };
        const sourceHashes = {
          detailedHash: buildSourceHash(detailedMarkdown),
          briefHash: buildSourceHash(briefMarkdown),
          bossHash: level === 'boss' ? buildSourceHash(bossMarkdown) : '',
        };
        const sourceHash = buildSourceHash({
          reportData,
          level,
          selected: workMarkdown,
          sourceReports,
        });
        ...
        const markdown = await createSmartReport({
          ...
          reportData,
          workMarkdown,
          options: { period, date, tool, project, level, platform, sourceReports },
        ...
        const record = saveSmartReportRecord(storeDir, {
          ...
          markdown,
          sourceHash,
          sourceReports: sourceHashes,
        });

        writeJson(res, 200, { markdown, record });
      });
      return;
    }
