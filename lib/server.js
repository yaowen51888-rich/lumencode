...
import { invalidateFileCache, getClaudeDirMaxMtime } from './cache.js';
...
import { checkForUpdate, getUpdateState, performUpdate } from './updater.js';
...
export function startServer(config, effectiveIncludeProjects, buildReportData, configPath) {
  ...
  function getCachedReport(cacheKey) {
    ...
    if (Date.now() >= cached.expire || cached.mtime !== getClaudeDirMaxMtime(config.claudeDir)) {
  ...
  function setCachedReport(cacheKey, data) {
    _reportCache.set(cacheKey, { data, expire: Date.now() + REPORT_CACHE_TTL, mtime: getClaudeDirMaxMtime(config.claudeDir) });
    // LRU: 超出限制时删除最早的条目
  ...
  async function getOrParse(config, includeProjects) {
    ...
    _parsedCachePromise = (async () => {
      try {
        ...
        _parsedCacheMtime = getClaudeDirMaxMtime(config.claudeDir);
        return result;
      } finally {
  ...
  async function buildSmartReportSource(params) {
    ...
    const __srcMtime = getClaudeDirMaxMtime(config.claudeDir);
  ...
  const server = createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/update/check') {
      try {
        ...
        writeJson(res, 200, await checkForUpdate(force));
      } catch (err) {
        console.error('API error:', err.message);
        writeJson(res, 500, { error: err.message || '服务器内部错误' });
      }
    ...
    if (url.pathname === '/api/update/status') {
      writeJson(res, 200, getUpdateState());
      return;
    }

    if (url.pathname === '/api/update/apply') {
      if (req.method !== 'POST') {
        writeJson(res, 405, { error: '仅支持 POST' });
        return;
      }
      // 防误触：npx/本地运行不支持自动更新，直接拒绝
      const info = await checkForUpdate();
      if (!info.globallyInstalled) {
        writeJson(res, 400, { error: 'npx/本地方式运行不支持自动更新，请执行 npm install -g lumencode@latest' });
        return;
      }
      writeJson(res, 200, performUpdate());
      return;
    }
  ...
  server.listen(PORT, '127.0.0.1', () => {
    // P0: 后台预热 parse + 首屏默认 report（daily/今天），首查命中热缓存，省冷读 ~2.6s + git/行级归因。
    // fire-and-forget，不阻塞 listen、不阻塞浏览器打开；getOrParse 的 in-flight 去重保证首查复用同一次 parse。
    // 测试用 LUMENCODE_PORT=0 绑随机端口，跳过预热——否则会调用注入的 mock buildReportData，污染调用计数
    if (process.env.LUMENCODE_PORT !== '0') {
      const _warmIp = computeIncludeProjects(config);
      const _warmDate = new Date().toISOString().slice(0, 10);
      getOrParse(config, _warmIp)
        .then(parsed => buildReportData('daily', _warmDate, config, _warmIp, 'all', parsed))
        .then(data => {
          if (data) setCachedReport(getReportCacheKey('daily', _warmDate, 'all', '', ''), data);
        })
        .catch(() => {});
    }
    ...
    checkForUpdate().then(r => {
      if (r.updateAvailable) console.log(`[updater] 发现新版本 v${r.latest}（当前 v${r.current}），可在设置页或右下角提示中更新`);
    }).catch(() => {});
  });
