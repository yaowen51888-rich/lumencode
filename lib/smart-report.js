...
function probeAgentOnce(definition) {
  return new Promise(resolve => {
    ...
    // 多字节字符（如中文）可能被 chunk 边界切开，toString('utf8') 会把不完整序列替换成 U+FFFD；
    // StringDecoder 跨 chunk 缓冲不完整字节，close 时 end() 冲出尾部
    ...
    child.on('close', code => {
      ...
      if (code === 0) {
        resolve({ detected: true, version: stdout.trim().split(/\r?\n/)[0] || '' });
      } else {
        // where.exe/command -v 退码非 0 = 命令不在 PATH；不回显 stderr——
        // zh-CN Windows 的 where.exe 输出 GBK（cp936），按 utf8 解会乱码。
        resolve({ detected: false, error: `not found in PATH (exit ${code})` });
      }
    });
  });
}
...
export async function checkAgentAvailable(definition, options = {}) {
  ...
  // ponytail: 智能体未安装是正常态（项目支持 15 工具，用户未必装全），
  // detected:false + error 已通过返回值透传给前端展示，无需控制台告警刷屏。
  return result;
}
