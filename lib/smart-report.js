...
const REQUIRED_SECTIONS = {
  ...
  workhorse: {
    brief: ['本期投入', '核心产出', '进展价值', '风险处理', '下一步计划'],
    detailed: ['本期投入', '核心产出', '进展价值', '风险处理', '下一步计划'],
  },
};

const UNSUPPORTED_CLAIM_PATTERNS = [
  { label: 'ROI', pattern: /\bROI\b|投资回报/i },
  { label: '节省时长', pattern: /节省.{0,8}(小时|工时|人天|人力|时长)/ },
  { label: '业务收益', pattern: /业务收益|营收|收入增长|商业价值提升/ },
  { label: '节省成本', pattern: /节省.{0,8}成本|降本增效/ },
];
...
export async function runSmartReportAgent(definition, prompt, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const promptAsArg = definition.promptAsArg === true;
    const spawnArgs = promptAsArg ? [...definition.args, prompt] : definition.args;
    const invocation = buildAgentSpawnInvocation(definition, spawnArgs);
    const child = spawn(invocation.command, invocation.args, {
...
export async function createSmartReport({
  ...
  if (qualityWarnings.length > 0) {
    throw new Error(`智能报告质量校验未通过: ${qualityWarnings.join('；')}`);
  }
  return normalized;
}
