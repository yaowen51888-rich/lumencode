...
> **Claude Code 帮你写了多少代码？一条命令见分晓。**

用数据回答老板的终极提问："这 AI 工具到底值不值？" —— Token 消耗、费用估算、AI 贡献度、代码产出，全部自动生成，还能一键复制成飞书/钉钉周报。
...
**写周报时** —— 打开 ccusage-report，选择周报，点击"工作汇报 → 复制"，粘贴到飞书群。完事。

**要证明 AI 投入产出比时** —— "上周 67% 的提交有 AI 参与，AI 辅助新增代码 4,200 行，费用 $12.5"。有数据，有底气。

**想了解自己怎么用 Claude Code 时** —— 哪个项目用得最多？哪个模型最费 Token？什么时候是编码高峰？一目了然。
...
- **工作汇报平台适配**：新增飞书、钉钉格式支持，一键切换输出格式
- **详报/简报模式**：工作汇报新增详报（详细）和简报（精简）两种输出模式
- **修复刷新布局跳动**：解决页面刷新时卡片先集中在中间、数据加载后才展开的问题
- **Markdown 渲染优化**：自定义列表标记、分隔线、表格悬停效果、代码片段样式
- **AI 贡献度增强**：改进置信度评分和文件级指标计算
- **暗色主题完善**：工作汇报区域、平台徽章等组件的暗色适配
