...
按月汇总，自动计算当月起止日期，适合月度效率复盘。支持趋势对比箭头，直观展示环比变化。
...
| 环比趋势箭头 | 统计卡片展示与上一周期的环比变化百分比 |
| 费用估算 | 基于模型定价自动计算预估 API 费用 |
| Token 明细 | 统计卡片展示输入/输出/缓存的 Token 构成 |
| 场景分析 | 按编码/测试/调试/文档/审查/规划分类工作类型 |
| 数据钻取 | 点击图表可下钻查看明细（模型按日分布、项目会话、场景匹配示例） |
| Git 统计 | 关联本地仓库，展示提交次数、代码行数变化，未配置时显示引导提示 |
| 工作汇报 | 一键生成 Markdown 格式的周报/月报，支持复制和下载 .md |
| CSV/PDF 导出 | 一键导出 CSV 数据文件或打印为 PDF |
| 暗色模式 | 支持亮色/暗色主题切换，自动记忆偏好 |
| URL 状态持久化 | 周期和日期保存在 URL hash 中，刷新不丢失 |
